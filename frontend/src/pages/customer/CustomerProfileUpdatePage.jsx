...
export const CustomerProfileUpdatePage = () => {
  ...
  const [address, setAddress] = useState(
    typeof user?.address === 'string'
      ? user.address
      : user?.address?.line1
      ? `${user.address.line1} ${user.address.city || ''} ${user.address.pincode || ''}`
      : ''
  );
        ...
        <Card className="bg-card p-6 rounded-2xl border border-border space-y-5 shadow-sm">
          ...
          <CardContent className="p-0 space-y-4 text-xs">
            ...
            <div>
              ...
              <div className="relative">
                <MapPin className="w-4 h-4 text-muted-foreground absolute left-4 top-3.5" />
                <textarea
                  rows={3}
                  value={address}
                  onChange={(e) => setAddress(e.target.value)}
                  placeholder="Enter complete delivery location address"
                  className="w-full bg-muted/40 border border-border focus:border-primary focus:ring-2 focus:ring-primary/20 rounded-2xl pl-11 pr-4 py-2.5 text-xs text-foreground placeholder-muted-foreground outline-none transition-all font-medium resize-none"
                />
              </div>
            </div>
          </CardContent>
...
export default CustomerProfileUpdatePage;
